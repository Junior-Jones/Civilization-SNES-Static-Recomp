/* Civilization-owned fixed 32-phase SNES S-DSP hardware model.
 *
...
#include <limits.h>
#include <string.h>
#include "civilization/civilization_dsp.h"
...
static void reg_mark(civ_dsp *d, uint8_t r) {
    d->reg_known[r >> 3u] |= (uint8_t)(1u << (r & 7u));
}
int civ_dsp_register_known(const civ_dsp *d, uint8_t r) {
    r &= 0x7fu;
...
static uint8_t reg_read(civ_dsp *d, uint8_t r) {
    d->last_latched_reg = (uint8_t)(r & 0x7fu);
    d->last_latch_phase = d->phase;
    return d->regs[r & 0x7fu];
}
static void reg_internal_write(civ_dsp *d, uint8_t r, uint8_t v) {
    r &= 0x7fu;
    d->regs[r] = v;
    reg_mark(d, r);
}
static uint8_t aram_read_optional(civ_dsp *d, uint16_t a, uint8_t *known) {
    d->aram_reads++;
...
static civ_dsp_stop_reason require_known(civ_dsp *d, uint8_t known) {
    if (known) return CIV_DSP_STOP_NONE;
    d->last_stop = CIV_DSP_STOP_ARAM_REQUIRED_UNKNOWN;
    return d->last_stop;
}
static void aram_write_value(civ_dsp *d, uint16_t a, uint8_t v, uint8_t known) {
...
static int check_counter(const civ_dsp *d, unsigned rate) {
...
static void update_counter(civ_dsp *d) { d->counter = d->counter ? (uint16_t)(d->counter - 1u) : 0x77ffu; }
...
static civ_dsp_stop_reason decode_brr_group(civ_dsp *d, civ_dsp_voice *v) {
    ...
    return CIV_DSP_STOP_NONE;
}

static void process_envelope(civ_dsp *d, civ_dsp_voice *v, unsigned index) {
    uint8_t base = (uint8_t)(index << 4u);
    int32_t env = v->envelope;
    if (v->envelope_mode == CIV_DSP_ENV_RELEASE) {
        env -= 8;
    ...
    }
    {
        ...
        if (v->envelope_mode == CIV_DSP_ENV_DECAY && (env >> 8) == (sustain >> 5u)) v->envelope_mode = CIV_DSP_ENV_SUSTAIN;
        v->previous_calculated_envelope = env;
        if (env < 0 || env > 0x7ff) {
            env = env < 0 ? 0 : 0x7ff;
            if (v->envelope_mode == CIV_DSP_ENV_ATTACK) v->envelope_mode = CIV_DSP_ENV_DECAY;
        }
        if (check_counter(d, rate)) v->envelope = env;
    }
}

static void voice_update_output(civ_dsp *d, unsigned index, int right) {
    uint8_t base = (uint8_t)(index << 4u);
...
static void voice_step1(civ_dsp *d, unsigned index) {
    uint8_t base = (uint8_t)(index << 4u);
    d->sample_address = (uint16_t)(((uint16_t)d->dir_latch << 8u) + (uint16_t)d->source_number * 4u);
    d->source_number = reg_read(d, (uint8_t)(base + 4u));
}
static void voice_step2(civ_dsp *d, unsigned index) {
    civ_dsp_voice *v = &d->voices[index];
...
static void voice_step3a(civ_dsp *d, unsigned index) {
    d->pitch |= (uint16_t)((reg_read(d, (uint8_t)((index << 4u) + 3u)) & 0x3fu) << 8u);
}
static void voice_step3b(civ_dsp *d, unsigned index) {
    civ_dsp_voice *v = &d->voices[index];
...
static civ_dsp_stop_reason voice_step3c(civ_dsp *d, unsigned index) {
    civ_dsp_voice *v = &d->voices[index];
    ...
    if ((reg_read(d, 0x6cu) & 0x80u) || (d->brr_header & 3u) == 1u) {
        v->envelope_mode = CIV_DSP_ENV_RELEASE;
        v->envelope = 0;
    }
    if (d->every_other_sample) {
        if (d->key_off & bit) { v->envelope_mode = CIV_DSP_ENV_RELEASE; d->key_off_events++; }
        if (d->key_on & bit) { v->key_on_delay = 5u; v->envelope_mode = CIV_DSP_ENV_ATTACK; d->key_on_events++; }
    }
    if (!v->key_on_delay) process_envelope(d, v, index);
    return CIV_DSP_STOP_NONE;
}
static civ_dsp_stop_reason voice_step3(civ_dsp *d, unsigned index) {
    civ_dsp_stop_reason s;
    voice_step3a(d,index);
    voice_step3b(d,index);
    s=voice_step3c(d,index);
    return s;
}
static civ_dsp_stop_reason voice_step4(civ_dsp *d, unsigned index) {
    civ_dsp_voice *v = &d->voices[index];
    d->looped = 0u;
    if (v->interpolation_position >= 0x4000u && v->active) {
        civ_dsp_stop_reason s = decode_brr_group(d, v);
        if (s != CIV_DSP_STOP_NONE) return s;
        if (v->brr_offset >= 7u) {
            if (d->brr_header & 1u) {
                if (!d->brr_next_known) return require_known(d, 0u);
                v->brr_address = d->brr_next_address;
...
static void voice_step5(civ_dsp *d, unsigned index) {
    civ_dsp_voice *v = &d->voices[index];
    uint8_t bit = (uint8_t)(1u << index);
    uint8_t x;
...
static void voice_step6(civ_dsp *d) { d->out_reg_buffer = (uint8_t)((uint32_t)d->voice_output >> 8u); }
static void voice_step7(civ_dsp *d, unsigned index) {
    reg_internal_write(d, 0x7cu, d->voice_end_buffer);
    d->env_reg_buffer = d->voices[index].env_out;
}
static void voice_step8(civ_dsp *d, unsigned index) { reg_internal_write(d, (uint8_t)((index << 4u) + 9u), d->out_reg_buffer); }
static void voice_step9(civ_dsp *d, unsigned index) { reg_internal_write(d, (uint8_t)((index << 4u) + 8u), d->env_reg_buffer); }

static int32_t fir_term(civ_dsp *d, unsigned tap, int ch, uint8_t *known) {
...
static void echo_step22(civ_dsp *d) {
...
static void echo_add_known(civ_dsp *d, unsigned tap, uint8_t *known_l, uint8_t *known_r) {
    d->echo_in[0]+=fir_term(d,tap,0,known_l); d->echo_in[1]+=fir_term(d,tap,1,known_r);
}
static void echo_step23(civ_dsp *d) {
...
static void echo_step24(civ_dsp *d) {
...
static void echo_step25(civ_dsp *d) {
    uint8_t kl=(uint8_t)((d->echo_enabled_latch&0x40u)!=0u),kr=(uint8_t)((d->echo_enabled_latch&0x80u)!=0u);
...
static civ_dsp_stop_reason push_pcm(civ_dsp *d,int16_t l,int16_t r,uint8_t known) {
    uint16_t w[2]; unsigned i,b;
    if(d->pcm_count>=CIV_DSP_PCM_FIFO_FRAMES){d->pcm_overflows++;d->last_stop=CIV_DSP_STOP_PCM_FIFO_OVERFLOW;return d->last_stop;}
    d->pcm_fifo[d->pcm_write_index*2u]=l;d->pcm_fifo[d->pcm_write_index*2u+1u]=r;
    d->pcm_known_fifo[d->pcm_write_index]=known?1u:0u;
    d->pcm_write_index=(d->pcm_write_index+1u)%CIV_DSP_PCM_FIFO_FRAMES;d->pcm_count++;d->pcm_frames_produced++;
    ...
    w[0]=(uint16_t)l;w[1]=(uint16_t)r;for(i=0;i<2u;i++)for(b=0;b<2u;b++){uint8_t x=(uint8_t)(w[i]>>(b*8u));d->pcm_fnv1a64^=x;d->pcm_fnv1a64*=UINT64_C(1099511628211);}return CIV_DSP_STOP_NONE;
}
static civ_dsp_stop_reason echo_step26(civ_dsp *d) {
    uint8_t kl=(uint8_t)((d->echo_enabled_latch&0x40u)!=0u),kr=(uint8_t)((d->echo_enabled_latch&0x80u)!=0u);
...
static civ_dsp_stop_reason echo_step27(civ_dsp *d) {
...
static void echo_step28(civ_dsp *d) { d->echo_enabled_latch=(uint8_t)((d->echo_enabled_latch&0xf0u)|((reg_read(d,0x6cu)&0x20u)?0u:1u)); }
static civ_dsp_stop_reason echo_write_word(civ_dsp *d,uint16_t a,int16_t x,uint8_t known) {
    int16_t e=(int16_t)(x&~1);
    aram_write_value(d,a,(uint8_t)e,known);
    aram_write_value(d,(uint16_t)(a+1u),(uint8_t)((uint16_t)e>>8u),known);
    return CIV_DSP_STOP_NONE;
}
static civ_dsp_stop_reason echo_step29(civ_dsp *d) {
    civ_dsp_stop_reason s;
    ...
    if(d->echo_enabled_latch&1u){s=echo_write_word(d,d->echo_pointer,clamp16(d->echo_out[0]),d->echo_out_known[0]);if(s!=CIV_DSP_STOP_NONE)return s;}
...
static civ_dsp_stop_reason echo_step30(civ_dsp *d) {
    civ_dsp_stop_reason s;if(d->echo_enabled_latch&1u){s=echo_write_word(d,(uint16_t)(d->echo_pointer+2u),clamp16(d->echo_out[1]),d->echo_out_known[1]);if(s!=CIV_DSP_STOP_NONE)return s;}d->echo_out[1]=0;d->echo_out_known[1]=1u;return CIV_DSP_STOP_NONE;
}

void civ_dsp_power_on(civ_dsp *d,uint8_t *aram,uint8_t *known) {
...
civ_dsp_stop_reason civ_dsp_write_register(civ_dsp *d,uint8_t r,uint8_t v) {
    if(!d) return CIV_DSP_STOP_INVALID_ARGUMENT;
...
civ_dsp_stop_reason civ_dsp_read_register(civ_dsp *d,uint8_t r,uint8_t *v) {
    if(!d||!v) return CIV_DSP_STOP_INVALID_ARGUMENT;
    r&=0x7fu;if(!civ_dsp_register_known(d,r)){d->last_stop=CIV_DSP_STOP_REGISTER_UNKNOWN;return d->last_stop;}*v=d->regs[r];d->register_reads++;return CIV_DSP_STOP_NONE;
}

civ_dsp_stop_reason civ_dsp_step_phase(civ_dsp *d) {
    civ_dsp_stop_reason s=CIV_DSP_STOP_NONE;uint8_t p;
    if(!d) return CIV_DSP_STOP_INVALID_ARGUMENT;
    if(d->last_stop!=CIV_DSP_STOP_NONE) return d->last_stop;
    p=d->phase;
    switch(p){
        case 0: voice_step5(d,0);voice_step2(d,1);break;
        case 1: voice_step6(d);s=voice_step3(d,1);break;
        case 2: voice_step7(d,0);if((s=voice_step4(d,1))==CIV_DSP_STOP_NONE)voice_step1(d,3);break;
        case 3: voice_step8(d,0);voice_step5(d,1);voice_step2(d,2);break;
        case 4: voice_step9(d,0);voice_step6(d);s=voice_step3(d,2);break;
        case 5: voice_step7(d,1);if((s=voice_step4(d,2))==CIV_DSP_STOP_NONE)voice_step1(d,4);break;
        case 6: voice_step8(d,1);voice_step5(d,2);voice_step2(d,3);break;
        case 7: voice_step9(d,1);voice_step6(d);s=voice_step3(d,3);break;
        case 8: voice_step7(d,2);if((s=voice_step4(d,3))==CIV_DSP_STOP_NONE)voice_step1(d,5);break;
        ...
        case 11: voice_step7(d,3);if((s=voice_step4(d,4))==CIV_DSP_STOP_NONE)voice_step1(d,6);break;
        ...
        case 14: voice_step7(d,4);if((s=voice_step4(d,5))==CIV_DSP_STOP_NONE)voice_step1(d,7);break;
        ...
        case 30:
            if(d->every_other_sample){d->key_on=d->new_key_on;d->key_off=reg_read(d,0x5cu);}update_counter(d);if(check_counter(d,reg_read(d,0x6cu)&0x1fu)){uint16_t n=(uint16_t)(((d->noise_lfsr<<14u)^(d->noise_lfsr<<13u))&0x4000u);d->noise_lfsr=(uint16_t)(n^(d->noise_lfsr>>1u));}s=voice_step3c(d,0);if(s==CIV_DSP_STOP_NONE)s=echo_step30(d);break;
        case 31: s=voice_step4(d,0);if(s==CIV_DSP_STOP_NONE)voice_step1(d,2);break;
        default: d->last_stop=CIV_DSP_STOP_PHASE_INVARIANT;return d->last_stop;
    }
    if(s!=CIV_DSP_STOP_NONE){d->last_stop=s;return s;}
    d->phase=(uint8_t)((p+1u)&31u);d->phase_steps++;if(d->phase==0u)d->sample_steps++;return CIV_DSP_STOP_NONE;
}
civ_dsp_stop_reason civ_dsp_step_smp_cycles(civ_dsp *d,uint32_t cycles){uint32_t i;if(!d)return CIV_DSP_STOP_INVALID_ARGUMENT;for(i=0;i<cycles;i++){civ_dsp_stop_reason s=civ_dsp_step_phase(d);if(s!=CIV_DSP_STOP_NONE)return s;}return CIV_DSP_STOP_NONE;}
civ_dsp_stop_reason civ_dsp_step_sample(civ_dsp *d){uint8_t start;uint32_t n=0;if(!d)return CIV_DSP_STOP_INVALID_ARGUMENT;start=d->phase;do{civ_dsp_stop_reason s=civ_dsp_step_phase(d);if(s!=CIV_DSP_STOP_NONE)return s;n++;}while(d->phase!=start&&n<=32u);return n==32u?CIV_DSP_STOP_NONE:CIV_DSP_STOP_PHASE_INVARIANT;}
size_t civ_dsp_pcm_available(const civ_dsp *d){return d?d->pcm_count:0u;}
size_t civ_dsp_pcm_read_with_knownness(civ_dsp *d,int16_t *o,uint8_t *known,size_t cap){size_t n=0;if(!d||!o)return 0u;while(n<cap&&d->pcm_count){o[n*2u]=d->pcm_fifo[d->pcm_read_index*2u];o[n*2u+1u]=d->pcm_fifo[d->pcm_read_index*2u+1u];if(known)known[n]=d->pcm_known_fifo[d->pcm_read_index];d->pcm_read_index=(d->pcm_read_index+1u)%CIV_DSP_PCM_FIFO_FRAMES;d->pcm_count--;n++;}return n;}
size_t civ_dsp_pcm_read(civ_dsp *d,int16_t *o,size_t cap){return civ_dsp_pcm_read_with_knownness(d,o,NULL,cap);}
uint64_t civ_dsp_pcm_hash(const civ_dsp *d){return d?d->pcm_fnv1a64:0u;}
uint8_t civ_dsp_phase(const civ_dsp *d){return d?d->phase:0xffu;}
