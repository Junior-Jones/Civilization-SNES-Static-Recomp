...
extern "C" uint8_t sc_static_apu_peek_dsp(uint8_t address){
  ...
  return (uint8_t)SC_STATIC_SNES::dsp.spc_dsp.read(address&0x7fu);
}
...
extern "C" int sc_static_apu_snapshot_save(void *data,size_t capacity){
  ...
  std::memcpy(h.magic,"SCAPU002",8u);h.version=2u;h.smp_size=(uint32_t)sizeof(SC_STATIC_SNES::SMP);h.aram_size=65536u;h.dsp_capacity=(uint32_t)kDspSnapshotCapacity;
...
extern "C" int sc_static_apu_snapshot_load(const void *data,size_t size,char *error,size_t error_capacity){
  ...
  if(std::memcmp(h.magic,"SCAPU002",8u)!=0||h.version!=2u||h.smp_size!=sizeof(SC_STATIC_SNES::SMP)||h.aram_size!=65536u||h.dsp_capacity!=kDspSnapshotCapacity){copy_error(error,error_capacity,"Static audio snapshot is from another build.");return 0;}
