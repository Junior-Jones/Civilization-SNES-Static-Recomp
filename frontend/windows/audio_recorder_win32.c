...
static int write_header(FILE *f,uint64_t frames){
    ...
    memset(h,0,sizeof(h));memcpy(h,"RIFF",4);le32(h+4,36u+data);memcpy(h+8,"WAVEfmt ",8);le32(h+16,16);le16(h+20,1);le16(h+22,2);le32(h+24,CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE);le32(h+28,CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE*4u);le16(h+32,4);le16(h+34,16);memcpy(h+36,"data",4);le32(h+40,data);return fflush(f)==0&&fseek(f,0,SEEK_SET)==0&&fwrite(h,1,44,f)==44;
}
void civilization_audio_recorder_win32_init(CivilizationAudioRecorderWin32 *r){if(r)memset(r,0,sizeof(*r));}
int civilization_audio_recorder_win32_start(CivilizationAudioRecorderWin32 *r,const wchar_t *dir){
    ...
    for(attempt=0;attempt<100u;attempt++){
        written=_snwprintf(r->path,4096,
            attempt==0u?
            L"%s\\Civilization-Full-Static-%04u%02u%02u-%02u%02u%02u-%03u.wav":
            L"%s\\Civilization-Full-Static-%04u%02u%02u-%02u%02u%02u-%03u-%02u.wav",
...
int civilization_audio_recorder_win32_write(CivilizationAudioRecorderWin32 *r,const int16_t *s,size_t frames){uint64_t max_frames=(UINT32_MAX-36u)/4u;if(!r||!r->file||!s)return 0;if(r->frames_written>max_frames||frames>max_frames-r->frames_written){r->write_failed=1;wcscpy(r->last_error,L"The WAV file reached its 4 GB format limit.");return 0;}if(fwrite(s,sizeof(int16_t)*2u,frames,r->file)!=frames){r->write_failed=1;wcscpy(r->last_error,L"WAV write failed.");return 0;}r->frames_written+=frames;return 1;}
int civilization_audio_recorder_win32_stop(CivilizationAudioRecorderWin32 *r){int ok;int write_failed;if(!r||!r->file)return 1;write_failed=r->write_failed;ok=write_header(r->file,r->frames_written);if(fclose(r->file)!=0)ok=0;r->file=NULL;r->write_failed=0;if(!ok&&!write_failed)wcscpy(r->last_error,L"Unable to finalize the WAV file.");return ok&&!write_failed;}
int civilization_audio_recorder_win32_active(const CivilizationAudioRecorderWin32 *r){return r&&r->file;}
