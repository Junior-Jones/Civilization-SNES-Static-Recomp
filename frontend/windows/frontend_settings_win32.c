...
#define SETTINGS_CLASS L"CivilizationFrontendSettings"
#define CONTROLS_CLASS L"CivilizationControls"
...
typedef struct DialogContext {
    CivilizationFrontendSettingsWin32 value;
    CivilizationFrontendSettingsWin32 *target;
    int accepted;
    HWND parent;
    HWND controls[CIVILIZATION_WIN_BINDING_COUNT];
    HWND gamepad_assignments[CIVILIZATION_WIN_BINDING_COUNT];
    HWND gamepad_changes[CIVILIZATION_WIN_BINDING_COUNT];
    HWND source;
...
static int keyboard_bindings_are_valid(
    const UINT bindings[CIVILIZATION_WIN_BINDING_COUNT]){
    int first;int second;
    if(!bindings)return 0;
    for(first=0;first<CIVILIZATION_WIN_BINDING_COUNT;first++){
        if(bindings[first]==0u||bindings[first]>0xfeu||
           keyboard_key_is_reserved(bindings[first]))return 0;
        for(second=first+1;second<CIVILIZATION_WIN_BINDING_COUNT;second++)
...
void civilization_frontend_settings_win32_load(CivilizationFrontendSettingsWin32 *s,
    ...
    if(!keyboard_bindings_are_valid(s->bindings)){
        CivilizationFrontendSettingsWin32 defaults;
        civilization_frontend_settings_win32_defaults(&defaults);
...
uint16_t civilization_frontend_settings_win32_input(
    const CivilizationFrontendSettingsWin32 *s, UINT key) {
    static const uint16_t mask[] = {CIVILIZATION_INPUT_UP,CIVILIZATION_INPUT_DOWN,
        CIVILIZATION_INPUT_LEFT,CIVILIZATION_INPUT_RIGHT,CIVILIZATION_INPUT_B,CIVILIZATION_INPUT_A,
        CIVILIZATION_INPUT_Y,CIVILIZATION_INPUT_X,CIVILIZATION_INPUT_L,CIVILIZATION_INPUT_R,
        CIVILIZATION_INPUT_START,CIVILIZATION_INPUT_SELECT};
    int i; if(!s)return 0;
    for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++)if(s->bindings[i]==key)return mask[i];
    return 0;
}
const wchar_t *civilization_frontend_settings_win32_action_name(int a){
    ...
    return a>=0&&a<CIVILIZATION_WIN_BINDING_COUNT?n[a]:L"Unknown";
}
void civilization_frontend_settings_win32_key_name(UINT key,wchar_t *text,size_t cap){
...
static void copy_keyboard_defaults(UINT destination[CIVILIZATION_WIN_BINDING_COUNT],
                                   int classic){
    CivilizationFrontendSettingsWin32 defaults;int i;
    if(classic)civilization_frontend_settings_win32_classic(&defaults);
    else civilization_frontend_settings_win32_defaults(&defaults);
    for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++)destination[i]=defaults.bindings[i];
}
...
static void update_controls_status(DialogContext *c){
    ...
    if(source==CIVILIZATION_INPUT_SOURCE_GAMEPAD){
        if(civilization_gamepad_win32_connected(c->gamepad))
            _snwprintf(text,384,L"Gamepad selected: %s. Keyboard shortcuts remain available.",
                       civilization_gamepad_win32_name(c->gamepad));
        else
...
static void refresh_control_bindings(DialogContext *c){
    ...
    source=(int)SendMessageW(c->source,CB_GETCURSEL,0,0);
    gamepad_connected=civilization_gamepad_win32_connected(c->gamepad);
    SetWindowTextW(c->assigned_header,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?
                   L"Current key - choose to change":L"Assigned input");
    ShowWindow(c->change_header,source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
    ShowWindow(c->keyboard_help_group,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?SW_SHOW:SW_HIDE);
    ShowWindow(c->keyboard_help_text,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?SW_SHOW:SW_HIDE);
    ShowWindow(c->gamepad_help_group,source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
    ShowWindow(c->gamepad_help_text,source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
    for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++){
        ShowWindow(c->controls[i],source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?SW_SHOW:SW_HIDE);
        ShowWindow(c->gamepad_assignments[i],source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
        ShowWindow(c->gamepad_changes[i],source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
        if(source==CIVILIZATION_INPUT_SOURCE_KEYBOARD){
            civilization_frontend_settings_win32_key_name(c->value.bindings[i],name,
                                                     sizeof(name)/sizeof(name[0]));
            SetWindowTextW(c->controls[i],name);
            EnableWindow(c->controls[i],c->keyboard_capture_action<0);
        }else{
            civilization_gamepad_win32_control_display_name(
    ...
    ShowWindow(c->accessible_defaults,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?SW_SHOW:SW_HIDE);
    ShowWindow(c->classic_defaults,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD?SW_SHOW:SW_HIDE);
    ShowWindow(c->gamepad_defaults,source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?SW_SHOW:SW_HIDE);
    EnableWindow(c->accessible_defaults,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD&&
                 c->keyboard_capture_action<0);
    EnableWindow(c->classic_defaults,source==CIVILIZATION_INPUT_SOURCE_KEYBOARD&&
                 c->keyboard_capture_action<0);
    EnableWindow(c->gamepad_defaults,source==CIVILIZATION_INPUT_SOURCE_GAMEPAD&&c->capture_action<0);
...
static int first_duplicate_binding(const DialogContext *c,int source){
    ...
    if(!c)return -1;
    for(first=0;first<CIVILIZATION_WIN_BINDING_COUNT;first++){
        a=source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?
            c->value.gamepad_bindings[first]:(int)c->value.bindings[first];
        for(second=first+1;second<CIVILIZATION_WIN_BINDING_COUNT;second++){
            b=source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?
...
static void start_gamepad_capture(HWND window,DialogContext *c,int action){
    ...
    if(!c||action<0||action>=CIVILIZATION_WIN_BINDING_COUNT)return;
    if(!civilization_gamepad_win32_connected(c->gamepad)){
    ...
    for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++)EnableWindow(c->gamepad_changes[i],FALSE);
    ...
    _snwprintf(message,256,L"Release all gamepad controls, then press the control for %s. Escape cancels.",
               civilization_frontend_settings_win32_action_name(action));
...
static void start_keyboard_capture(HWND window,DialogContext *c,int action){
    ...
    for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++)EnableWindow(c->controls[i],FALSE);
...
static LRESULT CALLBACK controls_proc(HWND w,UINT msg,WPARAM wp,LPARAM lp){
    ...
    case WM_CREATE:{
        ...
        for(i=0;i<CIVILIZATION_WIN_BINDING_COUNT;i++){
            set_font(CreateWindowW(L"STATIC",civilization_frontend_settings_win32_action_name(i),
            ...
            _snwprintf(change_label,96,L"Change %s...",
                       civilization_frontend_settings_win32_action_name(i));
            change_label[95]=L'\0';
    ...
    case WM_COMMAND:if(!c)break;switch(LOWORD(wp)){
        ...
        case ID_CONTROLS_GAMEPAD_DEFAULTS:
            civilization_gamepad_win32_default_bindings(c->value.gamepad_bindings);
            refresh_control_bindings(c);return 0;
        case ID_CONTROLS_APPLY:
            ...
            if(i>=0){
                ...
                SetFocus(c->value.input_source==CIVILIZATION_INPUT_SOURCE_GAMEPAD?
    ...
    case WM_TIMER:
        if(c&&wp==ID_GAMEPAD_CAPTURE_TIMER&&c->capture_action>=0){
            ...
            if(c->capture_wait_for_neutral){
                ...
                civilization_gamepad_win32_control_display_name(c->gamepad,control,
                                                           assignment,192);
                cancel_gamepad_capture(w,c,NULL);
                _snwprintf(status,320,L"%s is now assigned to %s. Choose Apply to save.",
                           civilization_frontend_settings_win32_action_name(action),assignment);
                status[319]=L'\0';set_controls_status(c,status);
    ...
    case WM_COMPLETE_KEYBOARD_CAPTURE:
        if(c&&c->keyboard_capture_action>=0){
            ...
            civilization_frontend_settings_win32_key_name((UINT)wp,key_name,
                                                     sizeof(key_name)/sizeof(key_name[0]));
            cancel_keyboard_capture(w,c,NULL);
            _snwprintf(status,320,L"%s is now assigned to %s. Choose Apply to save.",
                       civilization_frontend_settings_win32_action_name(action),key_name);
