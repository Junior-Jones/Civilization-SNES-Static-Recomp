...
#include "civilization_audio_output_dsound_win32.h"
...
#define APP_CLASS_NAME L"CivilizationStaticRecompWindow"
#define AUDIO_CLASS_NAME L"CivilizationStaticRecompAudioSettings"
#define SNAPSHOT_CLASS_NAME L"CivilizationStaticRecompSnapshotWindow"
#define INFO_CLASS_NAME L"CivilizationStaticRecompInformationWindow"
#define GETTING_STARTED_CLASS_NAME L"CivilizationGettingStartedWindow"
#define APP_TITLE L"Civilization (SNES)"
...
typedef struct AudioDialogState {
    CivilizationAudioSettings settings;
    HWND enabled;
    HWND device;
    HWND volume;
...
static HMENU g_menu;
static CivilizationRecomp *g_game;
...
static CivInputLatch g_keyboard_input;
static uint16_t g_gamepad_input;
static CivilizationGamepadInputWin32 g_gamepad;
...
static CivilizationAudioSettings g_audio_settings;
static CivilizationAudioOutput g_audio_output;
static CivilizationFrontendSettingsWin32 g_frontend_settings;
...
static int load_battery_sram_file_win32(CivilizationRecomp *game,
                                            const wchar_t *path,
                                            wchar_t *error,
    ...
    if (!civilization_recomp_sram_load(game, image, sizeof(image),
...
static int flush_battery_sram_win32(int force, wchar_t *saved_path,
    ...
    if (!force && !civilization_recomp_sram_dirty(g_game)) return 1;
    if (!ensure_directory_tree(g_saves_directory) ||
        !civilization_recomp_sram_copy(g_game, image, sizeof(image))) return 0;
    ...
    civilization_recomp_sram_mark_clean(g_game);
    g_sram_last_flush_frame = civilization_recomp_current_frame(g_game);
...
static void maybe_flush_battery_sram_win32(void) {
    uint32_t frame;
    if (!g_game || !civilization_recomp_sram_dirty(g_game)) return;
    frame = civilization_recomp_current_frame(g_game);
...
static void report_audio_diagnostic_events(void) {
    ...
    civilization_audio_output_get_diagnostics(&g_audio_output, &diagnostics);
    fifo_dropped = civilization_recomp_audio_dropped_frames(g_game);
    if (g_qpc_frequency.QuadPart > 0 && QueryPerformanceCounter(&fps_now)) {
        uint32_t current_frame = civilization_recomp_current_frame(g_game);
...
static uint16_t virtual_key_to_input(WPARAM key) {
    return civilization_frontend_settings_win32_input(&g_frontend_settings, (UINT)key);
}

static int keyboard_gameplay_active(void) {
    return g_frontend_settings.input_source == CIVILIZATION_INPUT_SOURCE_KEYBOARD ||
           !civilization_gamepad_win32_connected(&g_gamepad);
}

static uint16_t opposite_direction(uint16_t mask) {
    if (mask == CIVILIZATION_INPUT_UP) return CIVILIZATION_INPUT_DOWN;
    if (mask == CIVILIZATION_INPUT_DOWN) return CIVILIZATION_INPUT_UP;
    if (mask == CIVILIZATION_INPUT_LEFT) return CIVILIZATION_INPUT_RIGHT;
    if (mask == CIVILIZATION_INPUT_RIGHT) return CIVILIZATION_INPUT_LEFT;
    return 0u;
}

static uint16_t current_gameplay_input(void) {
    if (g_frontend_settings.input_source == CIVILIZATION_INPUT_SOURCE_GAMEPAD &&
        civilization_gamepad_win32_connected(&g_gamepad))
        return g_gamepad_input;
    return civ_input_latch_sample(&g_keyboard_input);
}
...
static int read_rom_file(const wchar_t *path, uint8_t **rom,
    ...
    buffer = (uint8_t *)malloc(CIVILIZATION_RECOMP_ROM_SIZE);
    if (!buffer) {
        (void)fclose(file);
        copy_wide(error, error_capacity,
    ...
    read_count = fread(buffer, 1u, CIVILIZATION_RECOMP_ROM_SIZE, file);
    trailing = fgetc(file);
    if (ferror(file) || fclose(file) != 0 ||
        read_count != CIVILIZATION_RECOMP_ROM_SIZE || trailing != EOF) {
        free(buffer);
        copy_wide(error, error_capacity,
                  L"The exact 1,572,864-byte Civilization (USA) NTSC ROM is required.");
        return 0;
    }
    *rom = buffer;
    *rom_size = read_count;
...
static void reset_game(void) {
    ...
    if (!civilization_recomp_reset(g_game, error, sizeof(error))) {
...
static void close_audio(void) {
    civilization_audio_output_close(&g_audio_output);
    if (g_game) (void)civilization_recomp_audio_discard(g_game);
}
...
static int open_audio(int show_error) {
    ...
    if (!civilization_audio_output_open(&g_audio_output, &g_audio_settings,
                                   g_window, error,
...
static void pause_game(const wchar_t *message) {
    civilization_audio_output_pause(&g_audio_output);
    g_paused = 1;
    civ_input_latch_reset(&g_keyboard_input);
    g_gamepad_input = 0u;
...
static void stop_game_on_core_failure(void) {
    ...
    g_gamepad_input = 0u;
    detail[0] = L'\0';
    utf8_to_wide(civilization_recomp_last_error(g_game), detail,
    ...
    if (log_written) {
        ...
        (void)_snwprintf(
            ...
            L"Civilization stopped because the static-recompiled core reached "
...
static void play_game(void) {
    ...
    civ_input_latch_reset(&g_keyboard_input);
    g_gamepad_input = 0u;
    civilization_audio_output_resume(&g_audio_output);
    reset_pacing_clock();
    if (civilization_audio_output_is_open(&g_audio_output)) {
        set_status(L"Civilization is running.");
    } else {
...
static void start_rom_load(int play_after_load) {
    ...
    set_status(L"Loading and verifying the exact Civilization ROM...");
    update_controls();
...
static int capture_core_screenshot_to(const wchar_t *base_directory,
    ...
    (void)_snwprintf(path, PATH_CAPACITY,
               L"%s\\civilization-frame-%08u-%04u%02u%02u-%02u%02u%02u-%03u.bmp",
...
static int capture_fullscreen_screenshot_to(wchar_t *saved_path,
    ...
    (void)_snwprintf(
        path, PATH_CAPACITY,
        L"%s\\civilization-fullscreen-%dx%d-frame-%08u-%04u%02u%02u-%02u%02u%02u-%03u.bmp",
...
static int write_static_core_failure_log(wchar_t *saved_path,
    ...
    if (!wide_to_utf8(log_path, narrow_log_path,
        ...
        !civilization_recomp_write_diagnostic_log(
...
static void capture_core_screenshot(void) {
    ...
    int resume_audio = g_game && !g_paused &&
                       civilization_audio_output_is_open(&g_audio_output);
    if (resume_audio) civilization_audio_output_pause(&g_audio_output);
    if (!(g_fullscreen_active ?
...
static void show_key_bindings(void) {
    ...
    if (civilization_frontend_controls_win32_dialog(
            g_window, g_instance, &g_frontend_settings, &g_gamepad)) {
        if (!civilization_frontend_settings_win32_save(
...
static const wchar_t g_welcome_text[] =
    L"Welcome to Civilization (SNES) Static Recompilation\r\n\r\n"
...
static int save_snapshot_slot(int slot) {
    ...
    if (!civilization_recomp_snapshot_save(g_game, narrow_path, error,
                                      sizeof(error))) {
    ...
    _snwprintf(status, sizeof(status) / sizeof(status[0]),
               ...
               civilization_recomp_current_frame(g_game), path);
    status[(sizeof(status) / sizeof(status[0])) - 1u] = L'\0';
    set_status(status);
    return 1;
}
...
static int load_snapshot_slot(int slot) {
    ...
    civilization_audio_output_pause(&g_audio_output);
    civilization_audio_output_flush(&g_audio_output);
    (void)civilization_recomp_audio_discard(g_game);
    if (!civilization_recomp_snapshot_load(g_game, narrow_path, error,
                                      sizeof(error))) {
    ...
    g_audio_last_queue_failures = 0u;
    g_audio_fps_window_qpc = 0u;
    g_audio_fps_window_frame = civilization_recomp_current_frame(g_game);
    g_presented_fps_window_count = g_presented_frame_count;
    g_pacing_render_resync_frames = 0u;
    g_loaded_snapshot_slot = slot;
    _snwprintf(status, sizeof(status) / sizeof(status[0]),
               L"Snapshot slot %d loaded at frame %u.", slot,
               civilization_recomp_current_frame(g_game));
    status[(sizeof(status) / sizeof(status[0])) - 1u] = L'\0';
    set_status(status);
    InvalidateRect(g_window, NULL, TRUE);
    return 1;
...
static void advance_frame_deadline(void) {
    ...
    if (g_qpc_remainder_accumulator >=
        CIVILIZATION_RECOMP_PRESENTATION_FPS_NUMERATOR) {
        frame_ticks += 1u;
...
static void pump_audio_during_render(CivilizationRecomp *game,
                                     void *opaque) {
    CivilizationAudioOutput *output = (CivilizationAudioOutput *)opaque;
    civilization_audio_output_pump_progress(output, game);
}
...
static int advance_one_frame(void) {
    ...
    if (!civilization_recomp_advance_streamed(
    ...
    if (keyboard_gameplay_active())
        civ_input_latch_consume(&g_keyboard_input, input_mask);
    civilization_audio_output_pump(&g_audio_output, g_game);
    report_audio_diagnostic_events();
    maybe_flush_battery_sram_win32();
    if (civilization_recomp_audio_overflowed(g_game)) {
        civilization_recomp_audio_clear_overflow(g_game);
    }
...
static void service_host_timer(void) {
    ...
    if (!g_frontend_settings.ntsc_frame_lock) {
        ...
        g_gamepad_input = civilization_gamepad_win32_poll(
            &g_gamepad, g_frontend_settings.gamepad_bindings);
        if (g_game && !g_paused &&
            civilization_audio_output_is_open(&g_audio_output)) {
            CivilizationAudioDiagnostics diagnostics;
            memset(&diagnostics, 0, sizeof(diagnostics));
            civilization_audio_output_get_diagnostics(&g_audio_output,
                                                &diagnostics);
...
static void paint_window(HWND window) {
    ...
    RECT client;
    RECT render_area;
    const uint32_t *pixels = civilization_recomp_frame_bgra(g_game);
    GetClientRect(window, &client);
    ...
    if (g_game && pixels && render_area.bottom > render_area.top) {
        ...
        if(g_fullscreen_active&&widescreen_frame) {
            ...
            while (scale > 1 &&
                   (frame_width * scale > available_width ||
                    (int)CIVILIZATION_RECOMP_FRAME_HEIGHT * scale >
                        available_height))
                --scale;
            if (frame_width * scale <= available_width &&
                ...
                draw_height = (int)CIVILIZATION_RECOMP_FRAME_HEIGHT * scale;
            } else {
                draw_width = available_width;
                draw_height = draw_width *
                              (int)CIVILIZATION_RECOMP_FRAME_HEIGHT / frame_width;
                if (draw_height > available_height) {
                    draw_height = available_height;
                    draw_width = draw_height * frame_width /
                                 (int)CIVILIZATION_RECOMP_FRAME_HEIGHT;
        ...
        bitmap.bmiHeader.biHeight = -(LONG)CIVILIZATION_RECOMP_FRAME_HEIGHT;
        bitmap.bmiHeader.biPlanes = 1u;
        bitmap.bmiHeader.biBitCount = 32u;
        bitmap.bmiHeader.biCompression = BI_RGB;
        ...
        if (StretchDIBits(dc, x, y, draw_width, draw_height,
                          0, 0,
                          frame_width,
                          CIVILIZATION_RECOMP_FRAME_HEIGHT,
                          pixels, &bitmap, DIB_RGB_COLORS, SRCCOPY) !=
            GDI_ERROR) {
            uint32_t emu_frame = civilization_recomp_current_frame(g_game);
        ...
        const wchar_t *message =
            L"Browse for the exact Civilization (USA) NTSC ROM and choose Run.";
        SetBkMode(dc, TRANSPARENT);
...
static int read_audio_dialog(AudioDialogState *state, HWND window) {
    ...
    if (selection > 0) {
        (void)SendMessageW(state->device, CB_GETLBTEXT, selection,
                           (LPARAM)state->settings.device_name);
        state->settings.device_name[CIVILIZATION_AUDIO_DEVICE_NAME_CAPACITY - 1u] =
            L'\0';
    }
    ...
    if (selection < CIVILIZATION_AUDIO_RESAMPLER_HERMITE ||
        selection > CIVILIZATION_AUDIO_RESAMPLER_NEAREST) return 0;
    state->settings.resampler_mode = (int)selection;
    if (!read_audio_edit_int(window, state->safety_buffer,
            L"Safety prebuffer", CIVILIZATION_AUDIO_MIN_SAFETY_BUFFER_MS,
            CIVILIZATION_AUDIO_MAX_SAFETY_BUFFER_MS,
            &state->settings.safety_buffer_ms) ||
        !read_audio_edit_int(window, state->drift_tolerance,
            L"Drift tolerance", CIVILIZATION_AUDIO_MIN_DRIFT_TOLERANCE_MS,
            CIVILIZATION_AUDIO_MAX_DRIFT_TOLERANCE_MS,
            &state->settings.drift_tolerance_ms) ||
        !read_audio_edit_int(window, state->max_rate_adjustment,
            L"Maximum rate correction",
            CIVILIZATION_AUDIO_MIN_RATE_ADJUSTMENT_PPM,
            CIVILIZATION_AUDIO_MAX_RATE_ADJUSTMENT_PPM,
            &state->settings.max_rate_adjustment_ppm) ||
        !read_audio_edit_int(window, state->averaging_frames,
            L"Averaging window", CIVILIZATION_AUDIO_MIN_AVERAGING_FRAMES,
            CIVILIZATION_AUDIO_MAX_AVERAGING_FRAMES,
            &state->settings.averaging_frames) ||
        !read_audio_edit_int(window, state->recovery_threshold,
            L"Recovery threshold", CIVILIZATION_AUDIO_MIN_RECOVERY_MS,
            CIVILIZATION_AUDIO_MAX_RECOVERY_MS,
            &state->settings.recovery_threshold_ms) ||
        !read_audio_edit_int(window, state->resume_fade,
            L"Resume fade", CIVILIZATION_AUDIO_MIN_FADE_MS,
            CIVILIZATION_AUDIO_MAX_FADE_MS,
...
static LRESULT CALLBACK audio_dialog_proc(HWND window, UINT message,
    ...
    switch (message) {
        ...
        case WM_CREATE: {
            ...
            if (!state) return -1;
            SetWindowTextW(window, L"Civilization Audio Settings");
            ...
            for (index = 0u; index < civilization_audio_device_count(); ++index) {
                if (civilization_audio_device_name(index, device_name,
        ...
        case WM_COMMAND:
            ...
            if (LOWORD(wparam) == ID_AUDIO_DEFAULTS) {
                civilization_audio_settings_defaults(&state->settings);
                apply_audio_settings_to_controls(state);
                return 0;
            }
...
static void show_audio_settings(void) {
    ...
    civilization_audio_output_get_diagnostics(&g_audio_output, &state.diagnostics);
    ...
    dialog = CreateWindowExW(
        WS_EX_DLGMODALFRAME | WS_EX_CONTROLPARENT,
        AUDIO_CLASS_NAME, L"Civilization Audio Settings",
    ...
    if (state.applied) {
        ...
        if (g_game) {
            if (open_audio(1) && civilization_audio_output_is_open(&g_audio_output))
...
static void initialize_paths_and_settings(void) {
    ...
    civilization_audio_settings_defaults(&g_audio_settings);
    civilization_audio_output_initialize(&g_audio_output);
    civilization_frontend_settings_win32_defaults(&g_frontend_settings);
    ...
    if (!unified_exists) {
        ...
        frontend_saved = civilization_frontend_settings_win32_save(
            &g_frontend_settings, g_settings_ini_path);
        civilization_audio_settings_save(&g_audio_settings, g_settings_ini_path);
    ...
    (void)civilization_gamepad_win32_initialize(&g_gamepad, NULL);
...
static void continue_startup_after_welcome(void) {
    ...
    if (g_adjacent_rom_found) {
        if (g_frontend_settings.auto_run_on_load) {
            set_status(L"ROM found in the Rom folder. Loading and starting Civilization.");
            start_rom_load(1);
        } else {
        ...
        set_status(L"Browse for the required Civilization ROM.");
        update_controls();
        SetFocus(g_browse_button);
        NotifyWinEvent(EVENT_OBJECT_FOCUS, g_browse_button,
                       OBJID_CLIENT, CHILDID_SELF);
    ...
    switch (message) {
        ...
        case WM_CREATE:
            ...
            g_status = CreateWindowExW(
                0, L"STATIC",
                L"Browse for the required Civilization ROM.",
                WS_CHILD | WS_VISIBLE | SS_LEFT | SS_NOPREFIX,
                ...
                case ID_ABOUT:
                    {
                        ...
                        show_information_window(L"About Civilization",
                                                about, 520, 320);
                    }
                    return 0;
        ...
        case WM_KEYUP:
            if (g_game && !g_paused && GetFocus() == g_window &&
                ...
                uint16_t mask = virtual_key_to_input(wparam);
                if (mask != 0u) {
                    civ_input_latch_release(&g_keyboard_input, mask);
                    return 0;
                }
            }
            break;
        ...
        case WM_APP_LOAD_COMPLETE: {
            ...
            if (g_close_requested) {
                if (result) {
                    civilization_recomp_destroy(result->game);
                    free(result);
                }
                DestroyWindow(window);
                return 0;
            ...
            if (g_play_after_load) {
                ...
                set_status(L"Civilization is loaded and ready. Choose Play to start.");
                SetFocus(g_pause_play_button);
            }
            return 0;
        }
        ...
        case WM_DESTROY:
            ...
            civilization_recomp_destroy(g_game);
            g_game = NULL;
            civilization_gamepad_win32_shutdown(&g_gamepad);
            PostQuitMessage(0);
            return 0;
...
int WINAPI wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE previous,
        ...
        g_qpc_frequency.QuadPart <= 0) return 1;
    {
        uint64_t scaled_qpc = (uint64_t)g_qpc_frequency.QuadPart *
            CIVILIZATION_RECOMP_PRESENTATION_FPS_DENOMINATOR;
