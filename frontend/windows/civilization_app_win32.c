...
#define ROM_INFO_CLASS_NAME L"CivilizationStaticRecompRomInformation"
...
static const wchar_t *ROM_REQUIREMENTS_TEXT =
    ...
    L"File size: 1,572,864 bytes\r\n"
    L"SHA-256: de2d5a952096c5f50368b9270d342aa6e7a39007ffbec27117e182e30ef4cf32\r\n\r\n"
...
static CivilizationAudioRecorderWin32 g_audio_recorder;
...
static int read_rom_file(const wchar_t *path, uint8_t **rom,
    ...
    if (ferror(file) || fclose(file) != 0 ||
        ...
        free(buffer);
        copy_wide(error, error_capacity,
                  L"The exact 1,572,864-byte Civilization (USA) ROM is required.");
        return 0;
    }
    *rom = buffer;
    *rom_size = read_count;
...
static void stop_game_on_core_failure(void) {
    ...
    civ_input_latch_reset(&g_keyboard_input);
    g_gamepad_input = 0u;
    set_status_utf8(civilization_recomp_last_error(g_game));
...
static int capture_window_screenshot_to(const wchar_t *base_directory,
    ...
    (void)_snwprintf(path, PATH_CAPACITY,
               L"%s\\civilization-window-frame-%08u-%04u%02u%02u-%02u%02u%02u-%03u.bmp",
...
static void show_game_controls(void) {
    wchar_t key_name[CIVILIZATION_WIN_BINDING_COUNT][64];
    wchar_t message[4096];
    int index;
    for (index = 0; index < CIVILIZATION_WIN_BINDING_COUNT; ++index)
        civilization_frontend_settings_win32_key_name(
            g_frontend_settings.bindings[index], key_name[index], 64u);
    (void)_snwprintf(message, sizeof(message) / sizeof(message[0]),
        L"Civilization SNES controls and current keyboard bindings\r\n\r\n"
    ...
    MessageBoxW(g_window, message, L"Civilization SNES Controls",
                MB_OK | MB_ICONINFORMATION);
}

static const wchar_t g_welcome_text[] =
    L"This launcher runs the statically recompiled Super Nintendo version of Civilization\r\n\r\n"
...
static int start_recording_to(const wchar_t *base_directory,
    ...
    if (civilization_audio_recorder_win32_active(&g_audio_recorder)) {
    ...
    if (!civilization_audio_recorder_win32_start(&g_audio_recorder, directory)) {
        (void)_snwprintf(message, message_capacity,
...
static int stop_recording(wchar_t *message, size_t message_capacity) {
    ...
    if (!civilization_audio_recorder_win32_stop(&g_audio_recorder)) {
        (void)_snwprintf(message, message_capacity,
    ...
    (void)_snwprintf(message, message_capacity,
                     ...
                     (double)frames / CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE, path);
    message[message_capacity - 1u] = L'\0';
    return 1;
}
...
static int advance_frame_batch(uint32_t frame_count) {
    ...
    if (headless_count &&
        !civilization_recomp_advance_headless(g_game, input_mask,
                                         headless_count, &result)) {
        stop_game_on_core_failure();
        return 0;
    }
    memset(&result, 0, sizeof(result));
    if (!civilization_recomp_advance(g_game, input_mask, 1u, &result)) {
        stop_game_on_core_failure();
        return 0;
    }
    if (keyboard_gameplay_active())
        civ_input_latch_consume(&g_keyboard_input, input_mask);
    recorder_was_active = civilization_audio_recorder_win32_active(&g_audio_recorder);
    civilization_audio_output_pump(&g_audio_output, &g_audio_recorder, g_game);
    if (recorder_was_active &&
        !civilization_audio_recorder_win32_active(&g_audio_recorder) &&
        g_audio_recorder.last_error[0]) {
...
static void paint_window(HWND window) {
    ...
    if (g_game && pixels && render_area.bottom > render_area.top) {
        ...
        int draw_height = draw_width * (int)CIVILIZATION_RECOMP_FRAME_HEIGHT /
                          (int)CIVILIZATION_RECOMP_FRAME_WIDTH;
        int x;
        int y;
        if (g_frontend_settings.integer_scale >= 1 &&
            g_frontend_settings.integer_scale <= 4) {
            draw_width = (int)CIVILIZATION_RECOMP_FRAME_WIDTH *
                         g_frontend_settings.integer_scale;
            draw_height = (int)CIVILIZATION_RECOMP_FRAME_HEIGHT *
                          g_frontend_settings.integer_scale;
        } else if (draw_height > available_height) {
            draw_height = available_height;
            draw_width = draw_height * (int)CIVILIZATION_RECOMP_FRAME_WIDTH /
        ...
        bitmap.bmiHeader.biWidth = (LONG)CIVILIZATION_RECOMP_FRAME_WIDTH;
        ...
        const wchar_t *message =
            L"Browse for the exact Civilization (USA) ROM and choose Run.\r\n"
    ...
    if (selection > 0) {
        ...
        if (SendMessageW(state->device, CB_GETLBTEXT,
            ...
            copy_wide(state->settings.device_name,
                      CIVILIZATION_AUDIO_DEVICE_NAME_CAPACITY, device_name);
        }
    }
    return 1;
}
    ...
    switch (message) {
        ...
        case WM_CREATE: {
            ...
            set_control_font(CreateWindowExW(
                0, L"STATIC",
                L"Choose how Civilization audio is played. Audio remains muted until Audio enabled is checked.",
                WS_CHILD | WS_VISIBLE | SS_LEFT,
            ...
            count = civilization_audio_device_count();
            for (index = 0u; index < count; ++index) {
                wchar_t name[CIVILIZATION_AUDIO_DEVICE_NAME_CAPACITY];
                if (civilization_audio_device_name(index, name,
                                              sizeof(name) /
                                              sizeof(name[0]))) {
...
static void show_audio_settings(void) {
    ...
    if (state.applied) {
        ...
        civilization_audio_settings_save(&g_audio_settings, g_audio_ini_path);
        if (g_game) {
...
static void initialize_paths_and_settings(void) {
    ...
    civilization_audio_recorder_win32_init(&g_audio_recorder);
    ...
    join_wide_path(g_sram_path,
                   ...
                   g_saves_directory, L"Civilization-USA.srm");
    ...
    civilization_audio_settings_load(&g_audio_settings, g_audio_ini_path);
    ...
    civilization_frontend_settings_win32_load(&g_frontend_settings,
                                         g_frontend_ini_path);
    ...
    (void)civilization_gamepad_win32_initialize(&g_gamepad, gamepad_database);
    if (!g_frontend_settings.input_source_saved)
        g_frontend_settings.input_source = g_gamepad.startup_gamepad_found ?
            CIVILIZATION_INPUT_SOURCE_GAMEPAD : CIVILIZATION_INPUT_SOURCE_KEYBOARD;
    SendMessageW(g_auto_run_checkbox, BM_SETCHECK,
...
static LRESULT CALLBACK rom_info_proc(HWND window, UINT message,
    ...
    switch (message) {
        case WM_CREATE: {
            SetWindowTextW(window, L"Required Civilization ROM");
...
static void show_first_run_rom_information(void) {
    ...
    dialog = CreateWindowExW(
        WS_EX_DLGMODALFRAME | WS_EX_CONTROLPARENT,
        ROM_INFO_CLASS_NAME, L"Required Civilization ROM",
        WS_CAPTION | WS_SYSMENU | WS_POPUP,
    ...
    switch (message) {
                ...
                case ID_ABOUT:
                    {
                        ...
                        (void)_snwprintf(about, sizeof(about) / sizeof(about[0]),
                            L"Civilization (SNES) Static Recomp\r\n\r\n"
                            L"Launcher file: Launcher.exe\r\n"
                            L"Game window: Civilization (SNES)\r\n\r\n"
        ...
        case WM_APP_LOAD_COMPLETE: {
            ...
            if (g_play_after_load) {
                ...
                set_status(L"Civilization (USA) is loaded and ready. Choose Play to start.");
                SetFocus(g_pause_play_button);
            }
            return 0;
        }
        ...
        case WM_DESTROY:
            ...
            (void)flush_battery_sram_win32(1, NULL, 0u);
            (void)civilization_audio_recorder_win32_stop(&g_audio_recorder);
            close_audio();
            if (g_loader_thread) {
