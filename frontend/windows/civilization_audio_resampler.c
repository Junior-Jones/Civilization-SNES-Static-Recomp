...
static int16_t interpolate(const double values[4], double position,
                           int mode) {
    if (mode == CIVILIZATION_AUDIO_RESAMPLER_NEAREST)
        return (int16_t)(position < 0.5 ? values[1] : values[2]);
    if (mode == CIVILIZATION_AUDIO_RESAMPLER_LINEAR)
...
void civilization_audio_resampler_set_mode(CivilizationAudioHermiteResampler *resampler,
                                     int mode) {
    if (!resampler) return;
    if (mode < CIVILIZATION_AUDIO_RESAMPLER_HERMITE ||
        mode > CIVILIZATION_AUDIO_RESAMPLER_NEAREST)
        mode = CIVILIZATION_AUDIO_RESAMPLER_HERMITE;
    resampler->mode = mode;
}
...
size_t civilization_audio_resampler_process(CivilizationAudioHermiteResampler *resampler,
    ...
    if (resampler->rate_ratio == 1.0) {
        ...
        if (queued > CIVILIZATION_AUDIO_RESAMPLER_PENDING_FRAMES -
                         resampler->pending_frames)
            queued = CIVILIZATION_AUDIO_RESAMPLER_PENDING_FRAMES -
                     resampler->pending_frames;
        if (queued) {
        ...
        while (resampler->fraction <= 1.0) {
            ...
            if (output_frames < output_capacity_frames) {
                ...
                output_frames++;
                       ...
                       CIVILIZATION_AUDIO_RESAMPLER_PENDING_FRAMES) {
