...
#define AUDIO_PREROLL_BUFFERS CIVILIZATION_AUDIO_BUFFER_COUNT
...
static UINT resolve_device_id(const wchar_t *device_name,
    ...
    UINT index;
    UINT count = civilization_audio_device_count();
    if (!device_name || !device_name[0]) {
        copy_wide(resolved_name, resolved_capacity, AUDIO_DEFAULT_DEVICE_LABEL);
        return WAVE_MAPPER;
    }
    for (index = 0u; index < count; ++index) {
        wchar_t candidate[CIVILIZATION_AUDIO_DEVICE_NAME_CAPACITY];
        if (civilization_audio_device_name(index, candidate,
                                      sizeof(candidate) /
                                      sizeof(candidate[0])) &&
...
static void release_finished_headers(CivilizationAudioOutput *output) {
    unsigned index;
    if (!output || !output->handle) return;
    for (index = 0u; index < CIVILIZATION_AUDIO_BUFFER_COUNT; ++index) {
...
static WAVEHDR *find_free_header(CivilizationAudioOutput *output) {
    unsigned index;
...
int civilization_audio_output_open(CivilizationAudioOutput *output,
    ...
    requested_frames =
        ((uint64_t)CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE *
         (uint64_t)settings->latency_ms) /
        (1000u * CIVILIZATION_AUDIO_BUFFER_COUNT);
    ...
    output->sample_storage = (int16_t *)calloc(
        (size_t)CIVILIZATION_AUDIO_BUFFER_COUNT * output->buffer_frames *
        CIVILIZATION_RECOMP_AUDIO_CHANNELS, sizeof(int16_t));
    ...
    output->pending_storage = (int16_t *)calloc(
        (size_t)output->buffer_frames * CIVILIZATION_RECOMP_AUDIO_CHANNELS,
        sizeof(int16_t));
    ...
    format.nChannels = CIVILIZATION_RECOMP_AUDIO_CHANNELS;
    format.nSamplesPerSec = CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE;
    format.wBitsPerSample = CIVILIZATION_RECOMP_AUDIO_BITS_PER_SAMPLE;
...
void civilization_audio_output_close(CivilizationAudioOutput *output) {
    if (!output) return;
    if (output->handle) {
        civilization_audio_output_flush(output);
...
void civilization_audio_output_resume(CivilizationAudioOutput *output) {
    ...
    output->fade_frames_remaining =
        (CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE * AUDIO_RESUME_FADE_MS) / 1000u;
    output->priming = 1;
    output->paused = 0;
}
...
static void apply_resume_fade(CivilizationAudioOutput *output, int16_t *samples,
                              size_t frame_count) {
    size_t frame;
    uint32_t fade_total =
        (CIVILIZATION_RECOMP_HOST_AUDIO_SAMPLE_RATE * AUDIO_RESUME_FADE_MS) / 1000u;
    ...
    for (frame = 0u; frame < frame_count &&
        ...
        unsigned channel;
        for (channel = 0u; channel < CIVILIZATION_RECOMP_AUDIO_CHANNELS; ++channel) {
            size_t sample = frame * CIVILIZATION_RECOMP_AUDIO_CHANNELS + channel;
...
void civilization_audio_output_pump(CivilizationAudioOutput *output,
    ...
    int16_t staging[4096u * CIVILIZATION_RECOMP_AUDIO_CHANNELS];
    if (!game) return;
    if (!output || !output->handle) {
        while (civilization_recomp_audio_available(game) > 0u) {
            ...
            size_t frames = civilization_recomp_audio_read(game, staging, request);
            if (!frames) break;
            if (recorder && civilization_audio_recorder_win32_active(recorder) &&
                !civilization_audio_recorder_win32_write(recorder, staging, frames))
                (void)civilization_audio_recorder_win32_stop(recorder);
        }
        return;
    }
    ...
    while (civilization_recomp_audio_available(game) > 0u) {
        ...
        MMRESULT result;
        if (!header) break;
        available = civilization_recomp_audio_available(game);
        request = output->buffer_frames - output->pending_frames;
        if (request > available) request = available;
        read_frames = civilization_recomp_audio_read(
            game, output->pending_storage +
                ((size_t)output->pending_frames * CIVILIZATION_RECOMP_AUDIO_CHANNELS),
            request);
        if (read_frames == 0u) break;
        if (recorder && civilization_audio_recorder_win32_active(recorder) &&
            !civilization_audio_recorder_win32_write(
                recorder, output->pending_storage +
                    ((size_t)output->pending_frames *
                     CIVILIZATION_RECOMP_AUDIO_CHANNELS), read_frames))
            (void)civilization_audio_recorder_win32_stop(recorder);
        ...
        apply_volume((int16_t *)header->lpData,
                     output->buffer_frames * CIVILIZATION_RECOMP_AUDIO_CHANNELS,
