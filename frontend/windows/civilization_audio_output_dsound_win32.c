...
#define AUDIO_BLOCK_ALIGN_BYTES \
    (CIVILIZATION_RECOMP_AUDIO_CHANNELS * (uint32_t)sizeof(int16_t))
...
void civilization_audio_settings_defaults(CivilizationAudioSettings *settings) {
    ...
    settings->resampler_mode = CIVILIZATION_AUDIO_RESAMPLER_HERMITE;
...
void civilization_audio_settings_load(CivilizationAudioSettings *settings,
                                 const wchar_t *ini_path) {
    if (!settings) return;
    civilization_audio_settings_defaults(settings);
...
static int restore_secondary_buffer(CivilizationAudioOutput *output) {
    DWORD status = 0u;
    HRESULT result;
...
static int clear_secondary_buffer(CivilizationAudioOutput *output) {
    void *first = NULL;
    void *second = NULL;
    DWORD first_size = 0u;
...
static void release_buffers(CivilizationAudioOutput *output) {
    if (!output) return;
...
int civilization_audio_output_open(CivilizationAudioOutput *output,
    ...
    civilization_audio_output_close(output);
    ...
    format.nChannels = (WORD)CIVILIZATION_RECOMP_AUDIO_CHANNELS;
    ...
    civilization_audio_resampler_set_rates(&output->resampler,
        (double)CIVILIZATION_RECOMP_AUDIO_SAMPLE_RATE,
        (double)output->device_sample_rate);
    civilization_audio_resampler_set_mode(&output->resampler,
                                    settings->resampler_mode);
    civilization_audio_resampler_reset(&output->resampler);
...
static void validate_write_cursor(CivilizationAudioOutput *output,
...
static int get_cursors(CivilizationAudioOutput *output,
...
static uint32_t queued_host_frames(CivilizationAudioOutput *output) {
    DWORD play_cursor;
    DWORD queued_bytes;
...
static uint32_t safe_queue_frames(CivilizationAudioOutput *output) {
    DWORD safe_write_cursor;
    DWORD queued_bytes;
    if (!output || !output->secondary_buffer) return 0u;
...
static void reset_rate_control(CivilizationAudioOutput *output) {
...
static int start_playback(CivilizationAudioOutput *output) {
    HRESULT result;
    if (!output || !output->secondary_buffer) return 0;
...
static void process_end_of_frame(CivilizationAudioOutput *output) {
...
static int write_secondary_buffer(CivilizationAudioOutput *output,
...
static void apply_volume(CivilizationAudioOutput *output, int16_t *samples,
                         size_t frame_count) {
    size_t frame;
...
static void pump_audio(CivilizationAudioOutput *output,
                       CivilizationRecomp *game, int end_of_frame) {
    int16_t native_staging[AUDIO_NATIVE_STAGING_FRAMES *
                           CIVILIZATION_RECOMP_AUDIO_CHANNELS];
    ...
    if (!game) return;
    while (civilization_recomp_audio_available(game) > 0u) {
        size_t available = civilization_recomp_audio_available(game);
        size_t request = available > AUDIO_NATIVE_STAGING_FRAMES ?
                         AUDIO_NATIVE_STAGING_FRAMES : available;
        size_t frames = civilization_recomp_audio_read(game, native_staging,
                                                 request);
        if (!frames) break;
        if (output && output->secondary_buffer) {
            size_t host_frames = civilization_audio_resampler_process(
