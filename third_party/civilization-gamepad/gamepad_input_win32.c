...
static void close_gamepad(CivilizationGamepadInputWin32 *input) {
    if (!input) return;
...
static int open_first_gamepad(CivilizationGamepadInputWin32 *input) {
...
uint16_t civilization_gamepad_win32_poll(
    ...
    static const uint16_t masks[CIVILIZATION_GAMEPAD_BINDING_COUNT] = {
        CIVILIZATION_INPUT_UP, CIVILIZATION_INPUT_DOWN, CIVILIZATION_INPUT_LEFT,
        CIVILIZATION_INPUT_RIGHT, CIVILIZATION_INPUT_B, CIVILIZATION_INPUT_A,
        CIVILIZATION_INPUT_Y, CIVILIZATION_INPUT_X, CIVILIZATION_INPUT_L,
        CIVILIZATION_INPUT_R, CIVILIZATION_INPUT_START, CIVILIZATION_INPUT_SELECT
    };
    uint16_t result = 0u;
    int index;
    ...
    for (index = 0; index < CIVILIZATION_GAMEPAD_BINDING_COUNT; ++index)
...
const wchar_t *civilization_gamepad_win32_name(const CivilizationGamepadInputWin32 *input) {
    if (!civilization_gamepad_win32_connected(input)) return L"No gamepad connected";
...
void civilization_gamepad_win32_control_display_name(
    ...
    base = civilization_gamepad_win32_control_name(control);
    if (input && civilization_gamepad_win32_connected(input)) {
        switch (control) {
